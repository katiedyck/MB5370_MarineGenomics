...
![](images/flye_graph.png)

**Observations**:

-   Largest contig (edge_29) = 4,681,059 bp; depth 21x. I'm guessing that this contig corresponds to the genome of the most abundant species in our mock community.
-   Contig 40 could also be a genome from our mock community (1,551,213 bp; depth 15x).
-   Contig 35 must be a repeat - it has a length of only 503 bp but a depth of 794x!
...
metabat has grouped contigs into 5 bins (exact numbers can vary from run to run which is why we used the `--seed` argument). If you compare the contigs in these bins with those shown in bandage you can see that *the largest and highest coverage contig has not been included*. Other than this some of the other bins seem to make some sense. For example you should see bins that contain similar groupings of contigs to those that you explored by hand using bandage + GC content and coverage.
...
- The bin with the highest contamination level is *metabat.5*. It was actually the only bin with any contamination level.